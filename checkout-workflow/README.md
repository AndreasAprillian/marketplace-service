...
Modul ini menggambarkan **BPMN proses checkout** marketplace yang dieksekusi dengan **Kogito (Apache KIE jBPM 10 / `jbpm-quarkus`)** di atas **Quarkus 3**. Alur diagram meniru `OrderWorkflowService.processOrder()` pada `order-service`, namun dieksekusi sebagai proses BPMN yang bisa dijalankan di test tanpa membutuhkan database atau Kafka.

- Port aplikasi: `8085`
- Dependency Kogito: `org.jbpm:jbpm-quarkus` (diatur oleh `org.kie.kogito:kogito-bom:10.2.0`)
...
| --- | --- | --- | --- |
| Validate Cart | `CartValidationService` | `validateCart(CheckoutRequest)` | Memastikan tiap `productId` di keranjang valid; bila tidak, memicu `orderProcesFailed` |
| Check Stock | `StockService` | `checkStock(...)` | Mengecek stok tiap item dan **mengurangi stok**; mengembalikan `boolean` |
| Publish Order Failed | `OrderFailedService` | `orderProcesFailed(CheckoutRequest)` | Menerbitkan event `ORDER_FAILED` (di sini diganti catatan log) |
